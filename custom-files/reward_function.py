    ...
    def reward_function(self, params):
        try:
            ...
            if inner_dist >= .25 and inner_dist <= .35:
                ...
                D = 1
                E = 0
            if prev_waypoint_index == len(waypoints)-1 or prev_waypoint_index == len(waypoints) - 2 or (prev_waypoint_index >= 0 and prev_waypoint_index <= 2):
                A = 2
                B = 1.1
                C = 5
                D = 0
            ...
            if prev_waypoint_index >= 26 and prev_waypoint_index <= 31:
                reward += C * (DC + SC) + DPC + (C * D * SQDC) + (E * DC) + (C * DC) + (C * E * SQDC)
            else:
